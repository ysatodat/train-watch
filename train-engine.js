(() => {
  ...
  function dirText(visit) { return visit.dir === 'down' ? 'つくば方面' : '秋葉原方面'; }
  function isOrigin(visit) { return (visit.dir === 'down' && visit.stationId === 'TX01') || (visit.dir === 'up' && visit.stationId === 'TX20'); }
  ...
  function fmtRemain(ms) {
    const s=Math.max(0,Math.ceil(ms/1000));
    return s<60 ? `あと${s}秒` : `あと${Math.floor(s/60)}分${String(s%60).padStart(2,'0')}秒`;
  }
  ...
  function buildVisits(now, stationId) {
    ...
    for (let h=8; h<=23; h++) {
      ...
      downBases.forEach(([kind,m]) => {
        ...
        out.push(targetFrom(minutes(h,m),inf.offset,baseDay,{kind,dir:'down',stationId,stop:DOWN[kind][station.i]!=null,approx:inf.approx,id:`d-${h}-${m}-${kind}-${stationId}`}));
      });
      upBases.forEach(([kind,m]) => {
        const inf=interpolate(UP[kind],station.i,UP.local); if(inf.offset==null) return;
        out.push(targetFrom(minutes(h,m),inf.offset,baseDay,{kind,dir:'up',stationId,stop:UP[kind][station.i]!=null,approx:inf.approx,id:`u-${h}-${m}-${kind}-${stationId}`}));
      });
    }
  ...
  function filterVisits(visits, filters={}) {
    const includePass = filters.includePass !== false;
    const dir = filters.dir || 'both';
    return visits.filter(v => (includePass || v.stop) && (dir === 'both' || v.dir === dir));
  }

  function focusForVisit(visit, now) {
    const nowMs = +now;
    const stationMs = +visit.stationAt;
    ...
    if (!visit.stop) {
      ...
      return {
        ...
        type:'pass',
        typeLabel:'通過',
        target:new Date(stationMs),
        status:active ? 'active' : (delta <= 180_000 ? 'soon' : 'future'),
        deltaMs:delta,
        approximate:true,
        activeUntil:new Date(stationMs + PASS_ACTIVE_AFTER_MS)
      };
    }
    ...
    if (nowMs < stationMs) {
      ...
      return {
        visit,
        key:`${visit.id}:arrival`,
        type:'arrival',
        typeLabel:'到着',
        target:new Date(arrivalMs),
        status:delta <= 0 ? 'active' : (delta <= 180_000 ? 'soon' : 'future'),
        deltaMs:delta,
        approximate:true,
        activeUntil:new Date(stationMs)
      };
    }
  ...
  function focusTitle(focus) {
    if (focus.type === 'arrival') {
      if (focus.status === 'active') return 'いま、停まるところ！';
      if (focus.deltaMs <= 30_000) return 'もうすぐ到着！';
      return 'つぎは到着';
    }
    if (focus.type === 'departure') {
      if (focus.status === 'active') return '発車の時間帯！';
      if (focus.deltaMs <= 30_000) return 'もうすぐ発車の時間！';
      return 'つぎは発車';
    }
    if (focus.status === 'active') return 'いま、通過！';
    if (focus.deltaMs <= 30_000) return 'もうすぐ通過！';
    return 'つぎは通過';
  }

  function focusMessage(focus) {
    if (focus.type === 'arrival') return focus.status === 'active' ? '停まる瞬間を見よう' : '電車が入ってくるよ';
    if (focus.type === 'departure') return focus.status === 'active' ? '動く瞬間を見よう' : '発車を見逃さないで';
    return focus.status === 'active' ? 'ビューン！' : '速い電車を見よう';
  }

  function focusCountdown(focus, now) {
    if (focus.status === 'active') return 'いま！';
    return fmtClock(+focus.target - +now);
  }

  window.TrainWatchEngine = {
    STATIONS, SERVICE,
    ARRIVAL_LEAD_MS, DEPARTURE_WINDOW_MS,
    stationById, dirText, isOrigin,
    fmtTime, fmtClock, fmtRemain,
    buildVisits, filterVisits, focusForVisit, getFocuses,
